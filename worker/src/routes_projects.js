...
export async function createProject({ env, session, request }) {
  ...
  await env.DB.prepare(
    "INSERT INTO projects (id, studio_id, name, status, created_at, updated_at) VALUES (?, ?, ?, 'active', ?, ?)"
  ).bind(pid, session.studioId, name, now, now).run();
  return json({ project: { id: pid, name, status: "active", created_at: now, updated_at: now } });
}

// Helper: load a project scoped to the caller's studio (tenancy guard).
export async function loadProject(env, session, pid) {
  return env.DB.prepare(
    "SELECT id, studio_id, name, status, created_at, updated_at FROM projects WHERE id = ? AND studio_id = ?"
...
export async function getProject({ env, session, params }) {
  ...
  return json({ project, floorplan: floorplan || null });
}
...
export async function updateProject({ env, session, params, request }) {
  ...
  const now = nowMs();
  await env.DB.prepare("UPDATE projects SET name = ?, status = ?, updated_at = ? WHERE id = ?")
    .bind(name, status, now, project.id).run();
  return json({ project: { ...project, name, status, updated_at: now } });
}
